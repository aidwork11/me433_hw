...
void init_camera(){
    ...
    OV7670_write_register(OV7670_REG_COM8, 0x00); // Disable AGC, AEC, AWB
    
    // Set fixed exposure and gain
    OV7670_write_register(OV7670_REG_AECH, 0x13);  // Set fixed exposure
    OV7670_write_register(OV7670_REG_GAIN, 0x00); // Set fixed gain
    OV7670_write_register(OV7670_REG_VREF, 0x03); // Set fixed VREF
    OV7670_write_register(OV7670_REG_COM9, 0x18); // Set fixed AGC ceiling
...
cameraPixel_t getPixel(int row, int col){
    int index = row*IMAGESIZEX+col;
    cameraPixel_t pixel;
    pixel.r = picture.r[index];
    pixel.g = picture.g[index];
    pixel.b = picture.b[index];
    return pixel;
}

void printPixel(cameraPixel_t pixel_left, cameraPixel_t pixel_right){

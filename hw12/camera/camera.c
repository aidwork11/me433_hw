...
void init_camera(){
    ...
    OV7670_write_register(0x12, 0x00);  // COM7: Reset all registers
    OV7670_write_register(0x11, 0x80);  // CLKRC: Use external clock directly
    OV7670_write_register(0x0C, 0x00);  // COM3: Disable scaling
    OV7670_write_register(0x3E, 0x00);  // COM14: Disable scaling
    OV7670_write_register(0x40, 0xD0);  // COM15: Set RGB565 format
    OV7670_write_register(0x12, 0x04);  // COM7: Set RGB output
...
void convertImage(){
    ...
    for(i=0;i<IMAGESIZEX*IMAGESIZEY*2;i=i+2){
        ...
        picture.b[picture.index] = cameraData[i]>>3;
        picture.g[picture.index] = ((cameraData[i]&0b111)<<3) | cameraData[i+1]>>5;
        picture.r[picture.index] = cameraData[i+1]&0b11111;
